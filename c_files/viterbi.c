  ...
  //this section pre-converts everything to log, I don't think I need to change anything
	...
	for (i = 1; i <= phmm->N; i++)
        ...
        for (t = 2; t <= T; t++) {
				...
				continue_update = 1;
				stop_early = 0;
				//same as above but find the earliest deletion in RDn's path so we can stop before then
				...
				//check RM1, if RM1 is more likely from RDn than what it currently is, change the value (subtract out the emission when comparing the transition)
